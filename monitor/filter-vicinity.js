...
function detectOverhead(conf, extra, aircraft, aircraftData) {
    ...
    if (Math.abs(overhead.overheadDistance) > conf.radius || Math.abs(overhead.overheadSeconds) > conf.time || overhead.overheadAltitude > conf.altitude) {
        // Check if it qualifies as vicinity instead
        if (conf.vicinity && aircraft.calculated.distance <= conf.vicinity.distance && aircraft.calculated.altitude <= conf.vicinity.altitude) {
            return {
                isVicinity: true,
                willIntersectOverhead: false,
                currentDistance: aircraft.calculated.distance,
...
module.exports = {
    ...
    evaluate: (aircraft) => aircraft.calculated.overhead.willIntersectOverhead || aircraft.calculated.overhead.isVicinity,
    sort: (a, b) => {
        const a_ = a.calculated.overhead;
        const b_ = b.calculated.overhead;
        // Overhead takes priority over vicinity
        if (a_.willIntersectOverhead !== b_.willIntersectOverhead) {
            return a_.willIntersectOverhead ? -1 : 1;
        }
        // For overhead, sort by time
        if (a_.willIntersectOverhead) {
            return a_.overheadTime - b_.overheadTime;
        }
        // For vicinity, sort by distance
        return a_.currentDistance - b_.currentDistance;
    },
    format: (aircraft) => {
        const { overhead } = aircraft.calculated;
        if (overhead.isVicinity) {
    ...
    debug: (type, aircraft) => {
        const { overhead } = aircraft.calculated;
        if (type == 'sorting') {
            if (overhead.isVicinity) return `vicinity, ${overhead.currentDistance.toFixed(1)}km`;
            return `${overhead.overheadFuture ? 'future' : 'past'}, ${overhead.overheadSeconds}s`;
        }
        return undefined;
    },
};
