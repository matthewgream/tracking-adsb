(function () {
    ...
    $(function init() {
        $.getJSON('./status.json').then(render).then(pollJSON).fail(handleError);
    });
    ...
    function update() {
        $.getJSON('./status.json').then(setPollInterval).then(render).fail(handleError);
    }
    ...
    function setPollInterval(data) {
        ...
        if (interval.setByData === false && typeof data.interval !== 'undefined') {
    ...
    function renderTranslations(opts) {
        ...
        $.each(['#status', '#map-link'], function (index, item) {
            var $item = $(item);
    ...
    function checkIsRunning(data) {
        ...
        if (typeof prevTime === 'undefined' || data.time > prevTime) {
            isRunning = true;
        }
